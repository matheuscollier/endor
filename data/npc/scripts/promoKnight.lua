			...
			if getPlayerVocation(cid) == tabela[x] then
				if getPlayerLevel(cid) >= 35 then
					...
					elseif (getPlayerVocation(cid) == 8) and getPlayerLevel(cid) >= 101 then
						npcHandler:say('Voce quer ser promovido para Knight Commander por 30.000gp?', cid)
						talkState[talkUser] = 1
						
					elseif (getPlayerVocation(cid) == 11) and getPlayerLevel(cid) >= 200 then
						npcHandler:say('Voce quer ser promovido para Knight Grand Cross por 200.000gp?', cid)
						talkState[talkUser] = 1
	...
	elseif(msgcontains(msg, 'yes') and talkState[talkUser] == 1) then
		...
		elseif(getPlayerVocation(cid) == 11) then
			if isPremium(cid) then
				...
				npcHandler:say("Voce tem que ter uma conta VIP para se tornar um Knight Grand Cross!", cid)
			end
			talkState[talkUser] = 0
